...
export default function LiquidEther({
  ...
  resolution = 0.5,
  isBounce = false,
  colors = ['#5227FF10', '#FF9FFC10', '#B19EEF10'],
  style = {},
  className = '',
  autoDemo = true,
  autoSpeed = 0.5,
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      setCoords(x, y) {
        ...
        // Si el cursor está fuera del contenedor, usar las coordenadas más cercanas al borde
        let nx = (x - rect.left) / rect.width;
        let ny = (y - rect.top) / rect.height;
        
        // Convertir las coordenadas normalizadas a coordenadas del espacio NDC (-1 a 1)
        const ndcX = nx * 2 - 1;
        const ndcY = -(ny * 2 - 1);
        
        this.coords.set(ndcX, ndcY);
...
`;
  ...
  <div ref={mountRef} className={`liquid-ether-container ${className || ''}`} style={style}>
    {/* Overlay de contenido por encima del canvas */}

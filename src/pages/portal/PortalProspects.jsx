...
export default function PortalProspects() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const { data } = await axios.get(`${API}/api/portal/plants`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        if (active) setPlants(data)
      } catch (e) {
        if (active) setError('No se pudo cargar la lista de plantas. Asegúrate de que el backend esté corriendo.')
      } finally {
  ...
  const handleSave = useCallback(async (id, patch) => {
    ...
    try {
      const token = await getToken()
      const { data } = await axios.patch(`${API}/api/portal/plants/${id}`, patch, {
        headers: { Authorization: `Bearer ${token}` },
      })
